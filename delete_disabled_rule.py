# Custom Build 1.1
# Deletes rules which are disabled.
...
print('###########################################################')
print('1. Report of rules which are disabled ')
print('2. Delete disabled rules')
print('###########################################################')
        ...
        for rule in response.json()['items']:
            ...
            if rule['enabled'] is False:
                ...
                if report_flag==1:
                    ...
                    r_write(rule,log)
                    to_delete_Rule_ID.append(rule["id"])
...
print("Number of rules which are disabled              : ",custom_rule_counter)
...
url ="https://" +  device + "/api/fmc_config/v1/domain/"+domain+"/policy/accesspolicies/"+acp_id[policy_id_1 -1]+"/accessrules?bulk=true&filter=ids:"
...
try:
    ...
    print("Processing, Please Wait!")
    n=20
    final = [to_delete_Rule_ID[i * n:(i + 1) * n] for i in range((len(to_delete_Rule_ID) + n - 1) // n )] 

    for i in final:

        delete_url=url+','.join(i)
